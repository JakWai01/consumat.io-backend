...
def set_favorite(tmdb: object, database: object, external_id: str, media: str,
    ...
    tv_code = None
    if media == "Season":
        result = tmdb.get_season_details(external_id, code, season_number)
        code = result.get("code")
        tv_code = result.get("tv_code")
    if media == "Episode":
        result = tmdb.get_episode_details(external_id, code, seasonNumber, episodeNumber)
        tv_code = tmdb.get_season_details(external_id, code, season_number).get("tv_code")
        code = result.get("code")
