...
class WatchCount:
    def get_watch_count(self, database, tmdb, user, type):
        count = 0
        if type in "MovieEpisodeTVSeason":
            ...
            results = MediaData.query.from_statement(
                text(
                    "SELECT * FROM media_data, user_data WHERE user_data.user_id_content = media_data.user_id_content_media_data AND user_data.external_id_content = :user;"
                )).params(user=user, type=type).all()
